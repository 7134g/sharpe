...
const (
	// DefaultCleanIntervalTime 默认协程清理时间
	DefaultCleanIntervalTime = 2 * time.Second

	// Waiting Working Idling 工作状态
	Waiting int = 0
	Working int = 1
	Idling  int = 2
)
...
// NewPool 新建一个厂房
func NewPool(size int32, prune bool, expiryTime time.Duration) *Pool {
	p := &Pool{
		capacity:       size,                   //厂房容量
		release:        make(chan struct{}, 1), //监察停止监控信号
		pruneDuration:  2 * time.Millisecond,   //清理间隔时间
		expiryDuration: expiryTime,             //工人空闲下班时间
		lock:           newSpinLock(),
	}
...
func (p *Pool) startWork(worker *worker) {
	go func() {
		//循环监听任务列表，一旦有任务立马取出运行,不close chan或条件return的话，该循环会一直执行
		for f := range worker.task {
			//进行工作
			f.TaskFunc(f.Param)
			//标记工作完成
			p.wg.Done()
...
// Close 关闭协程池，让所有worker退出运行以防goroutine泄露,必须先调用Wait
func (p *Pool) Close() {
	p.once.Do(func() {
		//通知清理协程退出
		close(p.release)
